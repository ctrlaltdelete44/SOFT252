...
public class Secretary extends Account implements Serializable, IViewAccounts{
    ...
    /**
     * secretary can view doctor and patient accounts at different times, so this
     * method allows for the different requirements
    ...
    /**
     * secretary can only remove patient accounts, so will always cleanly delete them
    ...
    public void removeAccount(String strAccount)
    {
        ...
        AccountSingleton.getOrCreate().remove(account);
    }
    ...
    {
        ArrayList<Request> list = RequestSingleton.getOrCreate().getRequests();
    ...
    /**
     * deleting a given request - for example if a patient tries to make multiple
     * appointments, secretary has the option to clear spam ones
    ...
    public void deleteRequest(String strRequest)
    {
        ...
        RequestSingleton.getOrCreate().remove(r);
    }
    
    /**
     * an alternative to the above for when the request reference is already known
    ...
    /**
     * each request type is authorised differently and this is handled by the request.
     * this method:
     *  - unregisters the request if associated with a patient
     *  - processes the request
     *  - notifies the request creator that the request was confirmed
     *  - returns an output message for the secretary
    ...
    /**
     * appointment requests are authorised in a different way and have more information associated with them
    ...
    {  
        ...
        RequestAdapter rAdapter = new RequestAdapter(request); AccountAdapter doctorAdapter = new AccountAdapter(assignedDoctor); DateAdapter dAdapter = new DateAdapter(assignedDate);
    ...
    {
        ArrayList<StockItem> list = StockSingleton.getOrCreate().getStock();
        
        String[] listData = new String[list.size()];
        
        for (int i = 0; i < list.size(); i++)
            listData[i] = list.get(i).getName() + ":" + list.get(i).getQuantity();
     ...
     * @param name - the item of stock to add
     ...
     * @return - returns the stock item to symbolise it being given to the patient
     */
