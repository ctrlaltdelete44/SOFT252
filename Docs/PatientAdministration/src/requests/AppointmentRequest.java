...
public class AppointmentRequest extends Request implements Serializable {
    ...
    /**
     * assigns the doctor and date requested by the patient - this is when
     * the request is considered complete, so is when it is tracked
    ...
    public void assignRequestedTarget(Doctor doctor, LocalDate date)
    {
        ...
        track();
        //System.out.println("Request assigned targets: " + doctor.getFirstName() + " " + doctor.getSurname() + ", " + date + ":APPOINTMENTREQUEST");
    }
    ...
    /**
     * before processing, this is assigned it's actual doctor and date
     * therefore this method simply needs to create a new appointment and
     * link it to the doctor and then the patient via the connect method
    ...
    /**
     * standard accessor
     * @return - doctor linked to request (either requested by patient or assigned
