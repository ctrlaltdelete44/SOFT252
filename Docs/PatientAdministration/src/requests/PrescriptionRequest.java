...
/**
 * request for a prescription - when a patient is assigned a prescription,
 * it follows the same pipeline as other requests as a patient would need to
 * visit a secretary to collect it
 * @author davie
 */
public class PrescriptionRequest extends Request implements Serializable {
    ...
    public void assignPrescription(Prescription p)
    {
        ...
        //System.out.println("Requesting prescription of " + prescription.getQuantity() + " " + prescription.getMedicine() + " dosage of " + prescription.getDosage() + ":PRESCRIPTIONREQUEST");
        track();
    }
    ...
    /**
     * on processing, this will check that the requested prescription is in stock.
     * then it will set the prescription associated with this request
     * to be the patients active prescription
     * @return - returns a positive or negative message based on whether the item is in stock
     */
    @Override
